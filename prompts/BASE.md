...
**При каждом вызове tool** в поле `content` ответа пиши краткое описание:
- Что ты собираешься делать и зачем
- Какой шаг из плана сейчас выполняется
- Что получилось на предыдущем шаге

Это отправляется владельцу в чат как прогресс-сообщение. Пиши **человеческим языком**, кратко (1-3 предложения). Примеры:

- "Читаю файл agent.py чтобы найти место для изменения."
- "Файл прочитан. Нашёл нужную функцию на строке 150. Сейчас напишу новый код и закоммичу."
- "git push не прошёл — конфликт. Проверяю git status чтобы понять причину."
- "Вторая попытка push после reset. Если не сработает — попробую другой подход."

**НЕ пиши пустой content** при вызовах tool. Всегда объясняй что делаешь.
